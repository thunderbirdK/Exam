...
Run netdiscover on the virtual network
...
Run a scan on 130 we know what is 52.100 and 52.130
...
Executed Hydra on the target machine and got SSH Credentials
un- user
PW - letmein

run a ls-l to list the files and direcotries

then in the kali environment run the rlogin in the remote host
where u get the login to the box.

the run cat /etc/passwd

now we can see that there is are two users
1. user 
2. vulnix

i guess vulnix has root permissions so we have to concentrate on the vulnix user.
